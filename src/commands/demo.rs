...
pub async fn demo(demo_data: DemoData, provider: Provider<Http>) {
    ...
    let secret = PrimitiveU256::from_str_radix(&demo_data.pk, 16)
        .expect("Error: failed to get u256 from secret.");
    let random_secret = rand::random::<u64>();
    let serct_fr = u256_to_fp(secret);
    let random_secret_fr = Fr::from_repr(FrRepr::from(random_secret))
    ...
    let mut leaves_strings: Vec<String> =
        serde_json::from_str(&white_list_data).expect("Error reding addresses.");
    let mut leaves_fr = leaves_strings
        .iter()
        .map(|x| {
            let num =

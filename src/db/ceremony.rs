...
impl CeremonyT {
    ...
    pub fn save_ceremony(&self, mut cfg: Config)->Result<()> {
        ...
        self.ceremonies.insert(key.to_be_bytes(), value.clone()).expect("db insert failed");
        self.ceremonies.insert("currect", value).expect("db insert failed");
        Ok(())
    }
    pub fn get_ceremony(&self, ceremonyId: Option<u64>) -> Result<Config>{
        match ceremonyId {
            Some(id) => {
                ...
                match cfg {
                    Some(data) => {
                        let (cfg, _) = bincode::decode_from_slice(&data, bincode::config::standard()).expect("Failed to decode config");
                        Ok(cfg)
                    },
